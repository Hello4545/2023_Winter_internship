...
# 구현 필요: 수면 시간 측정
# 가장 늦은 시간을 구하는 부분에 문제가 있음 --> 개선 필요

# Inputs
# weekdays
avg_total_sleep_time_wk = 12 * 60 * 60    # Average TST로 할지 total range of sleep time으로 할지 고민 필요
init_time_wk = [ "2024-01-22 02:30:12", "2024-01-23 23:45:12", "2024-01-24 01:15:12"] # ['2024-01-21 23:12:12', '2024-01-22 23:10:12']  #'%Y-%m-%d %H:%M:%S'
...
time_diff_from_midnight = timedelta(hours=7)
max_init_time_wk = None
for sleep_time_str in init_time_wk:
    sleep_time = datetime.strptime(sleep_time_str, '%Y-%m-%d %H:%M:%S')
    adjusted_sleep_time = sleep_time - time_diff_from_midnight
    if max_init_time_wk is None or adjusted_sleep_time.time() > max_init_time_wk.time():
        max_init_time_wk = adjusted_sleep_time
max_init_time_wk += time_diff_from_midnight
max_init_time_wk = max_init_time_wk.time()

min_end_time_wk = None
for sleep_time_str in end_time_wk:
    sleep_time = datetime.strptime(sleep_time_str, '%Y-%m-%d %H:%M:%S')
    if min_end_time_wk is None or sleep_time.time() < min_end_time_wk.time():
        min_end_time_wk = sleep_time
min_end_time_wk = min_end_time_wk.time()

# weekend
# max_init_time_wd = max(init_time_wd)
# min_end_time_wd = min(end_time_wd)
max_init_time_wd = None
for sleep_time_str in init_time_wd:
    sleep_time = datetime.strptime(sleep_time_str, '%Y-%m-%d %H:%M:%S')
    adjusted_sleep_time = sleep_time - time_diff_from_midnight
    # print(adjusted_sleep_time)
    if max_init_time_wd is None or adjusted_sleep_time.time() > max_init_time_wd.time():
        max_init_time_wd = adjusted_sleep_time
max_init_time_wd += time_diff_from_midnight
max_init_time_wd = max_init_time_wd.time()

min_end_time_wd = None
for sleep_time_str in end_time_wd:
    sleep_time = datetime.strptime(sleep_time_str, '%Y-%m-%d %H:%M:%S')
    if min_end_time_wd is None or sleep_time.time() < min_end_time_wd.time():
        min_end_time_wd = sleep_time
min_end_time_wd = min_end_time_wd.time()
